...
for txt in ROINRRD:
    ...
    tempdir = "/gpfs_projects/sriharsha.marupudi/Moments_of_Inertia_Measurements"
    data1_nrrd = os.path.join(tempdir,"img.nrrd")
    aligned_img_nrrd = os.path.join(tempdir,"aligned_img.nrrd")
    table_csv = os.path.join(tempdir, "table.csv")
    log_txt = os.path.join(tempdir, "log.txt")
    
    # TODO: from {file with your BoneJ wrapper} import compute_bonej_thickness
    data1,data1header1 = nrrd.read("/gpfs_projects/sriharsha.marupudi/Segmentation-07131-ROI.nrrd")
    ...
    macro_file = "/gpfs_projects/sriharsha.marupudi/Moments_of_Inertia.py"
    
    fiji_path = "~/Fiji.app/ImageJ-linux64" #home directory
    
    fiji_cmd = "".join([fiji_path, " --ij2", " --headless", " --run", " "+macro_file, 
                         " \'image="+"\""+data1_nrrd+"\"", ", log_txt="+"\""+log_txt+"\"", ", aligned_img_nrrd="+"\""+aligned_img_nrrd+"\"", 
                         ", table_csv="+"\""+table_csv+"\""+"\'"])
    ...
    img, header = nrrd.read("/gpfs_projects/sriharsha.marupudi/Moments_of_Inertia_Measurements/aligned_img.nrrd")
    print(img,header)
    with open("/gpfs_projects/sriharsha.marupudi/Moments_of_Inertia_Measurements/table.csv", "r",) as file:
